...
export x64_compiller_options=" -mwindows -s -m64 -Wl,--build-id=none -pipe -Wno-int-conversion -Wl,--gc-sections -Wall -DCOFF_STANDALONE -ffunction-sections -fno-asynchronous-unwind-tables -nostdlib -fno-ident -O2 -c -Wl,--no-seh -g2 "
export x86_compiller_options=" -mwindows -s -Wl,--build-id=none -pipe -Wno-int-conversion -Wl,--gc-sections -Wall -DCOFF_STANDALONE -ffunction-sections -fno-asynchronous-unwind-tables -nostdlib -fno-ident -O2 -c -Wl,--no-seh -g2 "
...
function create_api_hashes(){
	...
	for api in $(cat $file_winapis| sed 's/->.*//' | sort | uniq); do
		echo "#define $hash_lib_prolog"$(echo ${api^^}  | sed 's/\..*//' ) $(python3 $file_hasher_script $api $hash_random $hash_seed) >> $file_api_hashes_deinition;
	done
	for api in $(cat $file_winapis|sed 's/.*->//'| sed 's/<-.*//'); do
		initial_api=$(echo $api)
		echo "#define $hash_func_prolog"$(echo ${api^^}  | sed 's/.*->//' ) $(python3 $file_hasher_script $initial_api $hash_random $hash_seed) >>  $file_api_hashes_deinition;
	done
}
...
function create_mainpic_entry(){
	...
	for api in $(cat $file_winapis | sort | uniq); do
		...
		dll_hash_name=$(echo $hash_lib_prolog${api_dll^^} | sed 's/\..*//')
          	api_hash_name=$(echo $hash_func_prolog${api_name^^} | sed 's/\..*//')
		echo -e "\tApi._$api_name = getFunctionPtr($dll_hash_name, $api_hash_name);">>$file_mainpic_entry
	done
...
function build_api_resolvings(){
	...
	if [ "$arch" == "x64" ]; then
		${x64_compiller}-gcc ${x64_compiller_options} ${path_to_common}/api_resolve.c -c -o ${path_to_build}/api_resolve_x64.o
    	fi
	if [ "$arch" == "x86" ]; then
		${x86_compiller}-gcc ${x86_compiller_options} ${path_to_common}/api_resolve.c -c -o ${path_to_build}/api_resolve_x86.o
