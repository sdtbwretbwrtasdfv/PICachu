...
static uint64_t loadDll(unsigned long dll_hash) {
	...
	#ifdef H_LIB_WINHTTP
	else if (dll_hash == H_LIB_WINHTTP) {
	...
	#ifdef H_LIB_WININET
	else if (dll_hash == H_LIB_WININET) {
